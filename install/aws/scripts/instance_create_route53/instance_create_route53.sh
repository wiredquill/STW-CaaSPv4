...
HOSTED_ZONE_ID=$( aws route53 list-hosted-zones-by-name | grep -B 1 -e "susetech.org" | sed 's/.*hostedzone\/\([A-Za-z0-9]*\)\".*/\1/' | head -n 1 )
...
aws route53 change-resource-record-sets --hosted-zone-id "Z1YZ16JBG5SZ0Y" --cli-input-json "$INPUT_JSON"
